            ...
            <aside class="w-64 bg-white dark:bg-slate-900 border-r border-slate-100 dark:border-slate-800 flex flex-col transition-colors duration-300">
                ...
                <div class="h-16 px-5 flex items-center justify-between border-b border-slate-100 dark:border-slate-800 shrink-0">
                    <div class="flex items-center gap-2.5">
                        ...
                        <span class="font-bold text-sm text-slate-900 dark:text-white tracking-tight truncate">{{ auth.user?.empresa || auth.tenantName }}</span>
                    </div>
                </div>
                ...
                <!-- Section label + add button (solo para quienes pueden eliminar) -->
                <div class="px-5 pt-2 pb-1 flex items-center justify-between">
                    ...
                    <button v-if="auth.user.puedeEliminar" @click="showAddFolder = true" class="p-1 text-slate-400 hover:text-primary-500 hover:bg-primary-50 dark:hover:bg-primary-900/10 rounded-lg transition-all" title="Nueva carpeta">
                ...
                <nav class="flex-1 overflow-y-auto px-3 pb-4 space-y-0.5 custom-scrollbar">
                    ...
                    <div v-if="folders.length === 0" class="px-3 py-6 text-xs text-slate-400 dark:text-slate-600 text-center">Sin carpetas aún</div>
                </nav>
            </aside>
            ...
            <main class="flex-1 flex flex-col p-8 overflow-y-auto custom-scrollbar transition-colors">
                <header class="flex justify-between items-center mb-8 shrink-0">
                    <div class="flex items-center gap-2 flex-wrap">
                        <h1 class="text-base font-bold text-slate-900 dark:text-white">{{ selectedFolderName || 'Todos los íconos' }}</h1>
                    ...
                    <BaseButton v-if="auth.user.puedeEliminar" variant="primary" @click="showUpload = true">
                        <svg class="w-4.5 h-4.5 mr-2" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M12 5v14M5 12h14" /></svg>
                        Agregar Icono
                    </BaseButton>
                </header>
